...
// when each page's content last actually changed — see writeExtras()
const STAMP_FILE = 'sitemap-lastmod.json';
const STAMP_COMMENT =
  'generated by scripts/build.mjs — when each page last changed, by content hash. ' +
  'Committed on purpose: it is the only record of these dates, and a fresh clone ' +
  'would otherwise stamp every page with its own build day. Do not edit by hand.';
...
async function writeExtras(config, routes) {
  ...
  // lastmod describes the page, not the build. Stamping every route with
  // today's date told crawlers that all six pages changed whenever anything
  // was rebuilt — a redirect stub, a comment, nothing at all — which is the
  // fastest way to teach them the field is noise.
  //
  // So each route's built HTML is hashed and the stamp only moves when those
  // bytes actually move. The build is byte-deterministic (an unchanged page
  // hashes identically across runs), which is what makes this stable rather
  // than a slower way of saying "today".
  //
  // The table is committed, because it is the only memory of when a page last
  // changed: derive it from the working tree and a fresh clone would stamp
  // everything with the day it happened to build.
  const stampPath = join(ROOT, STAMP_FILE);
  const prev = JSON.parse(await readFile(stampPath, 'utf8').catch(() => '{}'));
  const today = new Date().toISOString().slice(0, 10);
  const stamps = { _comment: STAMP_COMMENT };
  const moved = [];
  for (const r of routes) {
    const html = await readFile(join(DIST, r.out));
    const hash = createHash('sha256').update(html).digest('hex').slice(0, 16);
    const was = prev[r.path];
    if (was && was.hash === hash) {
      stamps[r.path] = was;
    } else {
      stamps[r.path] = { hash, lastmod: today };
      moved.push(was ? `${r.path} ${was.lastmod} -> ${today}` : `${r.path} (new)`);
    }
  }
  await writeFile(stampPath, JSON.stringify(stamps, null, 2) + '\n');
  if (moved.length) log('sitemap lastmod moved:', moved.join(', '));
  else log('sitemap lastmod unchanged — no page content moved');

  const urls = routes
    .map(
      (r) => `  <url>
    <loc>${site.origin}${r.path}</loc>
    <lastmod>${stamps[r.path].lastmod}</lastmod>
